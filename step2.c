...
int main() 
{
   ...
   printf("Domain: %s:\n",domain);//domain name extracted from the URL
    ...
    printf("\n\nDone.\n\n");
   close(sock);// TCP connection closed after each object downloaded as it is Non-Persistent HTTP
   }
   gettimeofday(&tv2, NULL);// timer end time set after completion of downloading of all objects
   printf ("Total time = %f seconds\n",(double) (tv2.tv_usec - tv1.tv_usec) / 1000000 +(double) (tv2.tv_sec - tv1.tv_sec));// Total time required to download printed
    return 0;
}
